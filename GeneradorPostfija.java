...
public class GeneradorPostfija {
    ...
    public List<Token> convertir(){
        ...
        for(int i=0; i<infija.size(); i++){
            ...
            else if(t.tipo == TipoToken.PAR_AP){
                pila.push(t);
            }
            else if(t.tipo == TipoToken.PAR_CIE){
                while(!pila.isEmpty() && pila.peek().tipo != TipoToken.PAR_AP){
                    Token temp = pila.pop();
                    postfija.add(temp);
                }
                if(!pila.empty() && pila.peek().tipo == TipoToken.PAR_AP){
                    pila.pop();
                }

                // Esta sección de aquí es para manejar el ")" que cierra la
                // condición de la estructura de control
                if(estructuraDeControl && infija.get(i + 1).tipo == TipoToken.LLAVE_AP){
                    postfija.add(new Token(TipoToken.PUNT_COMA, ";", null));
                }
            }
            ...
            else if(t.tipo == TipoToken.PUNT_COMA){
                while(!pila.isEmpty() && pila.peek().tipo != TipoToken.LLAVE_AP){
                    Token temp = pila.pop();
                    postfija.add(temp);
                }
                postfija.add(t);
            }
            else if(t.tipo == TipoToken.LLAVE_AP){
                // Se mete a la pila, tal como el parentesis. Este paso
                // pudiera omitirse, sólo hay que tener cuidado en el manejo
                // del "}".
                pila.push(t);
            }
            else if(t.tipo == TipoToken.LLAVE_CIE && estructuraDeControl){
        ...
        postfija.add(infija.get(infija.size()-1));
        return postfija;
    }
}